...
private customer:CustomerProvider,
  ...
  ionViewWillEnter()
  {
    ...
    console.log('oO'+this.navParams.get('order'))
  ...
  // console.log('event entered with loc',location)
    ...
    // this.key=this.order.distributerID;
      ...
      console.warn(`${city}`,this.order)
      // console.log('oregin dist id '+`${this.key.distributerID}`)
      // oregin: dist loc when accepts

      this.customer.distTracker(`${city}`,this.order.distributerID)
        .then((loc)=>{
          console.warn('myDistination',city,loc);
          //dist loc
          ...
          this.updateDirection(parseFloat(this.oregin_latitude),parseFloat(this.oregin_longitude),
            parseFloat(this.destination_latitude),parseFloat(this.destination_longitude));
...
// });
  ...
  ionViewDidLoad(){
    ...
    // this.event.subscribe('cityLatlng',location=>{
      ...
      this.distributorProvider.getCurrentIpLocation(this.order.location.lat, this.order.location.lng).then(city=>{
        console.log('city',city)
      this.RefresherRef = firebase.database().ref("valid/"+`${city}`+"/" + this.order.distributerID);
      this.RefresherRef.on('value', (snapshot) => {
if(this.order.distributerID!=null){
        this.customer.distTracker(`${city}`, this.order.distributerID)
          .then((loc) => {
console.log('geo error loc',city,loc)
...
this.oregin_latitude=loc.lat
  ...
  {
    if(destinationLat!=null &&this.waypts !=null){
      console.warn('waypoints',this.waypts);
